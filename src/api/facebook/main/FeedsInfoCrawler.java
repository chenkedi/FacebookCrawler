					...
					if(seeds.size()!=0){
						for(Seeds temp : seeds){						
			    			...
			    			if(!feedsList.get(0).getStatus().equals("error")){
			    				...
			    				if(feedsList.get(0).getCodeMessage().equals("12") || feedsList.get(0).getCodeMessage().equals("1")){
			    					log.info(temp.getName()+"的历史feed出现Facebook自身链接的错误，Code： "+feedsList.get(0).getCodeMessage()+",绝大部分数据已采集完毕，将Crawed_feed状态更新为-1，继续采集下一个种子！");
				    				String sql="UPDATE seeds set crawed_feeds=? WHERE seeds_id=?";
				    				seedsDao.updateCrawed(sql,temp.getSeedsId(),-1);
				    				log.info("种子的Crawed_Feeds爬取状态更新为-1成功!\n\n");
			    				}
